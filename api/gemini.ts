 ...
 *
 * 심화해석 6종(크레딧제)도 결국 이 엔드포인트로 실제 생성 요청을 보낸다 — 크레딧
 * 차감은 이 파일이 아니라 그 요청 전에 클라이언트가 먼저 호출하는 api/credits.ts에서
 * 한 번만 일어난다(이 파일은 모델 폴백·재시도 때문에 한 번의 "생성"에도 여러 번 호출될
 * 수 있어, 크레딧 차감을 여기 두면 재시도할 때마다 중복 차감되는 버그가 생기기 때문).
 */
