/**
 * 서양 고전점성술 — 순수 데이터/타입 (별자리, 행성 정보, 하우스, 도시 좌표 등)
 *
 * astronomy-engine에 의존하는 실제 계산 함수(calculateAstrology 등)는 astrologyCalculator.ts에 있음.
 * 여기 있는 것들은 입력 폼의 도시 선택 드롭다운, 결과 화면 렌더링, 클릭 핸들러 등에서
 * astronomy-engine 없이도(즉, 계산 전에도) 필요한 값들이라 별도 파일로 분리했다 —
 * astronomy-engine(번들 119KB)을 초기 로딩 경로에서 빼고 실제 사주 제출 시점에만 받도록
 * 코드 스플리팅하기 위함(계획안.md 7-AS 참고).
 */
...
export const ASPECT_ANGLES: { type: AspectType; angle: number; nature: '길각' | '흉각' | '중립' }[] = [
...
export const ASPECT_ORB = 6; // 도(度) — 고전점성술 통용 오브를 단순화해 균일 적용
...
export function isKnownDstDate(year: number, month: number, day: number): boolean {
...
export function signOf(longitude: number): { signIndex: number; signDegree: number } {
...
export const TRANSIT_PLANETS: PlanetKey[] = ['sun', 'moon', 'mercury', 'venus', 'mars'];
